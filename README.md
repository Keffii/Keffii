...
- 🎵 [Wireless Music player](https://github.com/Keffii/wireless_music_player)  
   Web-based music player that can be controlled wirelessly via an ESP32 Bluetooth controller or directly through the web interface.  
   Handles playback, volume control, and real-time updates via SSE with MySQL storage and a Spring Boot backend.  
   *Tech:* Java, Spring Boot, MySQL, Hibernate, Thymeleaf, HTML/CSS, JavaScript, REST API, Grafana, SSE, Bluetooth.
...
A Spring Boot application that collects temperature data from IoT devices, stores the readings in a relational database, and visualizes the data with graphs on a website.  
...
- 🔑 [Password Cracker GUI](https://github.com/Keffii/password_cracker_dearimgui)  
   A graphical tool for importing .txt files containing SHA256/MD5 hashes to recover the original password.  
   *Tech:* C++, Dear ImGui  
